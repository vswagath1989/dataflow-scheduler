...
// CHECK-NEXT:             %[[CMPI_INIT:.*]] = arith.cmpi eq, %[[VAL_3]], %[[CONSTANT_4]] : index
// CHECK-NEXT:             ktdf.pipeline {
...
// Verify that map-reduction-partials correctly handles the scf.if initializer
// case for the SFU compute stage:
//   - The iter_arg initializer is an scf.if %cond -> (tensor<1x64xf16>) where
//     the then-branch yields tensor.empty() and the else-branch yields a
//     ktdf.read_from_fifo result (warm-start / partial-sum continuation).
//   - The pass hoists memref.alloc to the top of the stage body.
//   - The scf.if is rebuilt with no result type:
//       then: linalg.fill(%zero, %alloc)   (was tensor.empty + yield)
//       else: memref read_from_fifo + memref.copy into %alloc
//   - The iter_arg is stripped from the scf.for (no more iter_args / scf.yield).
//   - linalg.generic is rewritten to buffer semantics (memref ins + outs, no SSA result).
      ...
      ktdf.pipeline {
        ...
        ktdf.stage depends_in(%2#2) depends_out(%2#3) {
          scf.for %arg0 = %c0 to %c2 step %c1 {
            ...
            // Condition: first iteration (arg0 == 0) zero-fills; subsequent
            // iterations warm-start from a prior FIFO read.
            %is_first = arith.cmpi eq, %arg0, %c0_3 : index
            ktdf.pipeline {
              ...
              ktdf.stage depends_in(%3#2) depends_out(%3#3) {
                // scf.if initializer: first iteration zero-fills (tensor.empty),
                // subsequent iterations warm-start (ktdf.read_from_fifo).
                %init = scf.if %is_first -> (tensor<1x64xf16>) {
                  %empty = tensor.empty() : tensor<1x64xf16>
                  scf.yield %empty : tensor<1x64xf16>
                } else {
                  %warm = ktdf.read_from_fifo %3#0 : <"L1LU" -> "SFU", 64xf16> -> tensor<1x64xf16>
                  scf.yield %warm : tensor<1x64xf16>
                }
                %5 = scf.for %arg1 = %c0_3 to %c256 step %c1_4 iter_args(%arg2 = %init) -> (tensor<1x64xf16>) {
